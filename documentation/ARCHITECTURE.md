...
**Режимы запуска бота:**
- *Встроенный (рекомендуется для Railway):* веб-сервис на старте поднимает бота в
  фоновом потоке (`main.start_bot_thread` → `bot.run_in_thread`, polling с
  `stop_signals=None`). Один сервис = веб + бот. Достаточно задать `BOT_TOKEN` в
  переменных веб-сервиса. `API_BASE` встроенный бот по умолчанию берёт локально
  (`127.0.0.1:$PORT`).
- *Отдельным процессом:* `python bot.py` (см. `bot.toml`) как самостоятельный сервис.

⚠️ Только ОДИН экземпляр должен поллить токен (иначе Telegram отдаёт 409 Conflict):
либо встроенный, либо отдельный — не оба сразу.

**Переменные окружения бота:** `BOT_TOKEN`, `WEBAPP_URL`, `API_BASE` (по умолчанию = WEBAPP_URL,
во встроенном режиме = localhost), `CHANNEL_ID` (опц.), `STATE_DIR` (персистентность подписок).
